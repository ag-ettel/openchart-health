...
// - Blue dashed line for this hospital's observed value ("{facilityLabel}")
...
interface DistributionHistogramProps {
  ...
  /** "This hospital" or "This nursing home" */
  facilityLabel?: string;
  /** True when the measure uses CMS risk adjustment (O/E model) */
...
export function DistributionHistogram({
  ...
  const hasCI = ciLower !== null && ciUpper !== null;
  ...
  // Plain-language summary for AT users.
  const ciClause = hasCI
    ? `; credible interval ${formatValue(ciLower!, unit)} to ${formatValue(ciUpper!, unit)}`
    : "";
  const avgClause = hasExplicitNationalAvg
    ? `, national average ${formatValue(nationalAvg!, unit)}`
    : "";
  const ariaSummary = `${facilityLabel} ${formatValue(value, unit)}${avgClause}${ciClause}, shown against national distribution across ${distribution.total.toLocaleString("en-US")} facilities.`;

  return (
    <figure className="mt-3" aria-label={ariaSummary}>
      <figcaption className="sr-only">{ariaSummary}</figcaption>
      {/* Histogram bars */}
      <div className="relative" style={{ height: 72 }} role="presentation">
      ...
      <div className="relative mt-0.5 border-t border-gray-200" style={{ height: 14 }}>
        <span className="absolute left-0 text-xs text-gray-600" style={{ transform: "translateY(1px)" }}>
          {formatValue(rangeMin, unit)}
        </span>
        <span className="absolute right-0 text-xs text-gray-600" style={{ transform: "translateY(1px)" }}>
      ...
      <div className="mt-1.5 flex flex-wrap items-center justify-center gap-x-4 gap-y-0.5 text-xs">
        <span className="flex items-center gap-1.5 font-medium text-blue-600">
          ...
          {formatValue(value, unit)} · {facilityLabel}
        </span>
      ...
      <div className="mt-1.5 space-y-1">
        <p className="text-center text-xs text-gray-400">
          Distribution across {distribution.total.toLocaleString("en-US")} facilities
        </p>
        {hasCI ? (
          <p className="text-xs text-gray-400">
            <span className="mr-1 inline-block h-2.5 w-4 rounded-sm bg-blue-300 align-middle" />
            Blue shading shows the plausible range for this facility&apos;s result.
            {(() => {
              // Use CMS assessment first when available
              if (comparedToNational === "NO_DIFFERENT" || comparedToNational === "TOO_FEW_CASES") {
                return " CMS assesses this facility as no different from the national rate. The plausible range overlaps with the national average.";
              }
              if (comparedToNational === "BETTER") {
                return " CMS assesses this facility as better than the national rate.";
              }
              if (comparedToNational === "WORSE") {
                return " CMS assesses this facility as worse than the national rate.";
              }
          ...
          <p className="text-xs text-gray-400">
            {isRiskAdjusted
              ? "Interval estimates are not available for this measure. This is a risk-adjusted rate published by CMS; individual facility uncertainty bounds are not provided."
              : "Interval estimates are not available for this measure."}
          </p>
        )}
      </div>
    </figure>
